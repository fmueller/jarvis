...
// TODO consider renaming to MessagePanel
class MarkdownContentPanel(project: Project, markdown: String, backgroundColor: Color) : JPanel() {
    ...
    init {
        ...
        parseAndRenderMarkdown(markdown, backgroundColor)
    }

    // TODO add some tests for the parsing logic
    private fun parseAndRenderMarkdown(markdown: String, backgroundColor: Color) {
        ...
        while (matcher.find()) {
            ...
            if (matcher.start() > lastEnd) {
                ...
                addNonCodeContent(nonCodeMarkdown, backgroundColor)
            }

            // Add syntax-highlighted code
            val languageId = matcher.group(1)?.uppercase(Locale.getDefault())
            val code = matcher.group(2)
            addHighlightedCode(languageId ?: "plaintext", code, backgroundColor)
        ...
        if (lastEnd < markdown.length) {
            ...
            addNonCodeContent(remainingNonCodeMarkdown, backgroundColor)
        }
    }

    private fun addNonCodeContent(markdown: String, backgroundColor: Color) {
    ...
    private fun addHighlightedCode(languageId: String, code: String, backgroundColor: Color) {
        ...
        if (editor != null) {
            ...
            addNonCodeContent(code, backgroundColor)
        }
    }
